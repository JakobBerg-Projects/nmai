...
_KW = {
    ...
    "voucher": [
        ...
        "lançamento", "bokfør", "bokfor", "posteringer", "kontering",
        "purregebyr", "gebyr",
    ],
    "timesheet": [
        "timer", "timesheet", "hours", "tid", "stunden", "heures", "horas",
        "timeliste", "timeregistrering", "timeføring",
        "erfassen sie", "zeiterfassung", "arbeitszeit",
        "registre timer", "registrer timer",
    ],
    "order": [
    ...
    "bank_rec": [
        ...
        "bankavstemming", "bankutskrift", "bankkonto", "avstem bank",
    ],
    "supplier_inv": [
    ...
    "salary": [
        ...
        "salário", "salaire", "sueldo", "nómina", "lønnsslipp",
        "masse salariale", "nóminas",
        "folha de pagamento", "hoja de pago", "fiche de paie",
        "lohnabrechnung", "gehaltsabrechnung", "lønnsutbetaling",
        "lønnsregistrering", "lønnstrekk", "lønnstype",
    ],
}
...
def classify_task(prompt: str) -> TaskType:
    ...
    # Multi-step lifecycle tasks — match FIRST before individual entity keywords
    lifecycle_kw = ["prosjektsyklus", "project lifecycle", "project cycle", "projektzyklus",
                    "cycle de projet", "ciclo del proyecto", "ciclo do projeto",
                    "gjennomfør heile prosjekt", "gjennomfør hele prosjekt",
                    "full prosjekt", "complete project"]
    if any(kw in p for kw in lifecycle_kw):
    ...
    # Voucher before payment — "bokfør purregebyr" is a voucher task, not payment/invoice
    # If text has explicit voucher keywords like bokfør/purregebyr/bilag, prioritize voucher
    if _has(p, _KW["voucher"]):
        # Only override invoice if it's a posting/booking task, not an actual invoice creation
        voucher_words = ["bokfør", "bokfor", "bilag", "posteringer", "kontering", "purregebyr", "gebyr"]
        if any(w in p for w in voucher_words):
    ...
    if _has(p, _KW["payment"]) and not _has(p, _KW["employee"]) and not _has(p, _KW["supplier"]):
        return TaskType.REGISTER_PAYMENT

    if _has(p, _KW["order"]) and not _has(p, _KW["invoice"]):
        return TaskType.CREATE_ORDER
...
SECTION_MAP: dict[TaskType, list[str]] = {
    ...
    TaskType.REGISTER_PAYMENT: ["payment"],
...
FAST_PATH_ELIGIBLE = {
    ...
    TaskType.OPENING_BALANCE,
    TaskType.YEAR_END_CLOSING,
    # Excluded from fast path (need full loop for accuracy):
    # CREATE_CREDIT_NOTE — needs to find correct invoice first
    # CREATE_REMINDER — needs to find correct invoice first
    # CREATE_TIMESHEET — complex multi-step (employee + project + activity + entries)
    # CREATE_PROJECT — complex lifecycle tasks
    # CREATE_INVOICE — complex workflows
}
